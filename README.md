...
```
novaproxy/
├── main.go                     # نقطه ورود اپلیکیشن دسکتاپ
├── app.go                      # ارتباط UI ↔ هسته اصلی
├── core_runtime.go             # هسته بدون رابط کاربری (حالت --core)
├── core_client.go              # کلاینت RPC برای زیرپردازه هسته
├── core_api.go                 # API های هسته
│
├── proxy/                      # هسته‌های اصلی پروکسی
│   ├── proxy.go                # سرور HTTP/SOCKS5، هندلر MITM
│   ├── gsa.go                  # مدیریت GSA
│   ├── gsa_relay.go            # موتور رله GSA
│   ├── gsa_lan.go              # اشتراک در شبکه محلی
│   ├── gsa_constants.go        # ثابت‌ها و جدول مسیریابی
│   ├── tls_fragment.go         # تکه تکه کردن TLS
│   ├── cert_verify.go          # تأیید گواهی پیشرفته
│   ├── cf_pool.go              # مخزن IP های کلودفلر
│   ├── config.go               # مدل‌های کانفیگ
│   ├── auto_routing.go         # مسیریابی خودکار دامنه
│   ├── doh.go                  # DNS-over-HTTPS
│   ├── port_manager.go         # مدیریت پورت
│   └── procmon.go              # مانیتورینگ پردازه‌ها
│
├── cert/                       # گواهی MITM
│   ├── cert.go                 # تولید و مدیریت CA
│   ├── installer.go            # نصب CA در سیستم‌عامل
│   └── exec_windows.go         # اجرای فرمان در ویندوز
│
├── server/                     # کدهای سمت سرور
│   ├── Code.gs                 # Google Apps Script
│   └── worker.js               # Cloudflare Worker
│
├── utls/                       # انشعاب refraction-networking/utls
├── sysproxy/                   # تنظیمات پروکسی سیستم (ویندوز)
├── frontend/                   # رابط کاربری React (TypeScript)
├── build/                      # آیکون و فایل‌های build
